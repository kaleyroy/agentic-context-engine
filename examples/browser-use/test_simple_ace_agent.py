...
async def main():
    ...
    agent = ACEAgent(
        llm=llm,  # Browser execution LLM
        ace_model=LITE_LLM_MODEL,  # ACE learning LLM (optional, this is the default)
    )
    ...
    print("-" * 50)
    try:
        await agent.run(task="在Cnblogs.com首页上查找48小时阅读排行第1的文章")
    ...
    print("-" * 50)
    try:
        await agent.run(task="在Cnblogs.com首页上查找48小时阅读排行第2的文章")
        print(f"✅ Completed! Learned {len(agent.playbook.bullets())} strategies\n")
    except Exception as e:
        print(f"⚠️  Task failed: {e}\n")
    ...
    print("-" * 50)
    try:
        await agent.run(task="在Cnblogs.com首页上查找48小时阅读排行第3的文章")
        print(f"✅ Completed! Learned {len(agent.playbook.bullets())} strategies\n")
    except Exception as e:
        print(f"⚠️  Task failed: {e}\n")
