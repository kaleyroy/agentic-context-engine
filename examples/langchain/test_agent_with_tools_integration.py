...
ace_model_name = "dashscope/qwen-plus"
...
def example_agent_with_tools():
    ...
    # chain = prompt | llm
    # chain_with_tools = prompt | llm.bind_tools([add, multiply, get_word_length])

    # Define langchain agent
    custom_agent = create_agent(
        model=llm,
        tools=[add, multiply, get_word_length],
        system_prompt="You are a helpful assistant. Think step by step and use tools as needed.",
    )
    print(
        f"Agent Type: {type(custom_agent).__name__}, {isinstance(custom_agent, CompiledStateGraph)}"
    )

    # Wrap with ACE
    # ace_agent = ACELangChain(runnable=chain, ace_model=ace_model_name, is_learning=True)
    # ace_agent = ACELangChain(runnable=chain_with_tools, ace_model=ace_model_name, is_learning=True)
    playbook_file = Path("agent_with_tools_learned.json")
    ace_agent = ACELangChain(
        runnable=custom_agent,
        ace_model=ace_model_name,
        is_learning=True,
        playbook_path=(
            "agent_with_tools_learned.json" if playbook_file.exists() else None
        ),
    )
...
Think through each step carefully."""
    ...
    # Wrap with ACE
    ace_agent = ACELangChain(runnable=chain, ace_model=ace_model_name)
